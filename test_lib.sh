...
docker run -ti -v`pwd`/tests:/home/tests -e SIMD=${simd} -e CUDAVER=${cudaver} -e SQAODVER=${version} --name ${container_name}  shinmorino/sqaod_buildenv:${dist} bash -c 'cd /home/tests; ./test_lib.sh $SQAODVER $SIMD $CUDAVER'
